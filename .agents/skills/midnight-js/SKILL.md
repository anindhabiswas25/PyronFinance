...
## 12.5) API drift found during M1 build (2026-08-28) — verified against installed 4.0.2/3.0.0/2.5.0

The wallet construction pattern in §5 above (`new WalletFacade(shielded, unshielded, dust); await
facade.start(...)`) **does not match the actually-installed packages this skill pins in §14.**
Found by reading `node_modules/@midnight-ntwrk/wallet-sdk-facade/dist/index.d.ts` directly, since
the official wallet SDK docs are still "coming soon." Corrected pattern:

```typescript
// WalletFacade's constructor is PRIVATE. Construction is via a static async factory that takes
// FACTORY FUNCTIONS for each sub-wallet, not pre-built instances:
const facade = await WalletFacade.init({
  configuration: {
    networkId: getNetworkId(),
    indexerClientConnection: { indexerHttpUrl, indexerWsUrl },
    relayURL: new URL(rpcUrl.replace(/^http/, 'ws')),      // submission config
    provingServerUrl: new URL(proofServerUrl),              // proving config (top-level, NOT
  },                                                         // per-sub-wallet as §5 implied)
  shielded: (config) => ShieldedWallet(config).startWithSecretKeys(shieldedSecretKeys),
  unshielded: (config) => UnshieldedWallet(config).startWithPublicKey(publicKey),
  dust: (config) => DustWallet(config).startWithSecretKey(dustSecretKey, dustParams),
});

// Also: ShieldedWallet(config)/UnshieldedWallet(config)/DustWallet(config) each take ONLY
// { networkId, indexerClientConnection[, batchSize] } — NOT provingServerUrl/relayURL/
// costParameters individually per §5's example. Those live solely on the facade-level
// `configuration` object above, which is a union of all sub-configs.

// PublicKey for UnshieldedWallet.startWithPublicKey is NOT a class with .fromKeyStore(...).toAddress()
// — it's `PublicKey.fromKeyStore(keystore)` returning `{ publicKey, addressHex, address: string }`.
// The unshielded address is the plain `.address` STRING PROPERTY, not a method call.
const publicKey = PublicKey.fromKeyStore(unshieldedKeystore);
const unshieldedAddress = publicKey.address; // NOT publicKey.toAddress()
```

`levelPrivateStateProvider`'s config (§7) also drifted — the installed version (4.0.2) requires
**at-rest encryption**, not present in the skill's original example:

```typescript
levelPrivateStateProvider({
  midnightDbName: 'my-app-db',
  privateStateStoreName: 'my-app-private-state',
  signingKeyStoreName: 'my-app-signing-keys',
  privateStoragePasswordProvider: async () => password,  // >= 16 chars, NEW requirement
  accountId: walletAddress,                                // NEW requirement, scopes storage
});
```

**A TypeScript dual-module-hazard**, not a real logic bug: passing `MidnightProviders`-shaped
providers into `deployContract`/`findDeployedContract` under `moduleResolution: "NodeNext"` can
fail to typecheck with an error like `Type 'ZKIR' is not assignable to type 'typeof ZKIR'. Two
different types with this name exist, but they are unrelated.` This happens because
`midnight-js-types`'s `ZKIR` brand (`unique symbol`) gets loaded once via this file's own ESM
import graph and once via a dependency's CJS import graph, producing two nominally distinct
types for the same real value. A narrow `providers as any` at the `deployContract`/
`findDeployedContract` call site is the pragmatic fix — the providers object itself typechecks
fine in isolation; only the cross-package brand identity is the problem.

**Circuit ID helper**: `ImpureCircuitId` is not exported by the installed `compact-js` (2.5.0).
Derive circuit IDs from the generated contract's own `ProvableCircuits<PS>` type instead:
`type MyCircuits = keyof GeneratedProvableCircuits<PS> & string;` — this also matches what
`deployContract`/`findDeployedContract` actually key against internally
(`Contract.ProvableCircuitId<C> = keyof C['provableCircuits'] & string`).

**`CompiledContract.withVacantWitnesses` vs `withWitnesses`**: §3's example uses
`withVacantWitnesses`, which is only valid for contracts that declare **no** witnesses (like the
counter tutorial). Any contract with real witnesses must use
`CompiledContract.withWitnesses(yourWitnessImplementation)` instead, or compilation fails.

None of the above was runnable end-to-end in this build environment (no funded Preprod wallet, no
running proof server — see docs/ROADMAP.md M1 status). Everything above is confirmed by reading
the installed `.d.ts` files directly and by a full `tsc --noEmit` pass across `packages/sdk` and
`scripts/`, both of which now typecheck cleanly. Re-verify against a live proof server before
trusting this section blindly — file structure was inferred from types, not observed behavior.
