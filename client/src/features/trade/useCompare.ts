...
export function compareQuotesOf(state: Pick<RfqState, 'order' | 'quotes'>, now: number): CompareQuote[] {
  return state.order
...
export function useCompareQuotes(state: RfqState, now: number): CompareQuote[] {
  const { order, quotes } = state;
  return useMemo(() => compareQuotesOf({ order, quotes }, now), [order, quotes, now]);
}
