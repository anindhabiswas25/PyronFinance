...
/**
 * The landing page's behaviour layer — a direct port of landing/main.js.
 *
 * It stays DOM-query driven rather than becoming per-component React state,
 * because that is what the original is: text splitting rewrites the contents of
 * a heading into per-word spans, line splitting has to measure rendered
 * geometry, and the slider translates a track. Rebuilding those as controlled
 * React would change their timing and, with it, the feel of the page. Mounting
 * once and running the original logic against the server-rendered markup is
 * both the smallest diff and the faithful one.
 *
 * Everything here no-ops under `prefers-reduced-motion: reduce`.
 */
export function LandingEffects() {
  useEffect(() => {
    ...
    /* Wheel input drives a target offset that the page eases towards, so
       scrolling glides to a stop instead of snapping. Touch devices keep their
       native (already inertial) scrolling. */
    ...
    const initTextSplitting = () => {
      document.querySelectorAll("[create-spans]").forEach(splitWords);
      document.querySelectorAll("[create-spans-body]").forEach(splitLines);
    };
    ...
    const initTextReveals = () => {
      document.querySelectorAll('[view-text="true"]').forEach((element) => {
    ...
    const initBlockReveals = () => {
      ...
      document.querySelectorAll(".view-element").forEach((el) => observer.observe(el));
      cleanups.push(() => observer.disconnect());
    };
    ...
    /* Card artwork is a looping Lottie (SVG renderer). Each animation is only
       built once its card is near the viewport, and it pauses whenever it
       scrolls back out so offscreen cards cost nothing. */
    type LottieHost = Element & {_anim?: {play(): void; pause(): void; destroy(): void; goToAndStop(v: number, f: boolean): void}};

    const initLottie = () => {
      const holders = document.querySelectorAll<LottieHost>("[data-lottie]");
    ...
    // lottie.min.js loads via next/script, which may land after this effect.
    ...
    const onResize = () => {
      ...
      resizeTimer = window.setTimeout(() => {
        document.querySelectorAll("[create-spans-body]").forEach((el) => {
          const revealed = el.querySelector(".line-span-body.view");
          splitLines(el);
          if (revealed) {
            el.querySelectorAll(".line-span-body").forEach((l) => l.classList.add("view"));
          }
        });
      }, 200);
    };
...
type LottieGlobal = {
  ...
  }): {play(): void; pause(): void; destroy(): void; goToAndStop(v: number, f: boolean): void};
};
